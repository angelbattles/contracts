...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, AccessControl {
    ...
    // Token name
    string private _name = 'Angel Battles 2 Token';
    ...
    function mintABToken(
        ...
        require(_cardSeriesId < 100, 'Card series must be less than 100');
        ...
        abcard.lastBattleTime = uint64(block.timestamp);
        abcard.lastBreedingTime = uint64(block.timestamp);
    ...
    function increasePower(uint256 tokenId, uint8 _amount)
        external
        onlySERAPHIM
    {
        ABCard storage abcard = ABTokenCollection[tokenId];
        if (abcard.tokenId == tokenId) {
            abcard.power += _amount;
        }
    }

    function getBreedingCount(uint256 tokenId) public view returns (uint8) {
        ABCard storage abcard = ABTokenCollection[tokenId];
        if (abcard.tokenId == tokenId) {
            return abcard.breedingCount;
        }
    }

    function incrementBreedingCount(uint256 tokenId) external onlySERAPHIM {
        ABCard storage abcard = ABTokenCollection[tokenId];
        if (abcard.tokenId == tokenId) {
            abcard.breedingCount++;
        }
    }
    ...
    function getLastBreedingTime(uint256 tokenId) public view returns (uint64) {
        ABCard storage abcard = ABTokenCollection[tokenId];
        if (abcard.tokenId == tokenId) {
            return abcard.lastBreedingTime;
        }
    }
