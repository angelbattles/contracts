...
contract Halo is ERC20, ERC20Permit, ERC20Votes {
    address public streamDropContract = address(0);
    ...
    // Returns the suppply currently available.
    // Tokens are minted one time in the initialize function, but gradually unlock over 10 years.
    // Total supply also does not include tokens in the 0 address. 
    function totalSupply() public view virtual override returns (uint256) {
        address deadAddress = 0x000000000000000000000000000000000000dEaD;
        return
            uint256(_maxSupply()) -
            balanceOf(streamDropContract) -
    ...
    // Initialize function can only be called by angelbattles.eth and can only be called once
    // The max supply of 1 billion tokens are sent to the streamDrop contract which
    // streams the tokens over 5 years or 10 years (treasury)
    function initialize(address _streamDropContract)
        public
    {
        ...
        isInitialized = true;
        streamDropContract = _streamDropContract;
        _mint(_streamDropContract, _maxSupply());
    }
}
